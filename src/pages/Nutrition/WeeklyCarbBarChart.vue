    ...
    methods: {
        async fetchCarbsData() {
            ...
            try {
                ...
                console.error('Error fetching weekly carb data:', error);
            }
        },
        setChartData(weeklyData) {
            const days = weeklyData.map(item => new Date(item.forDay).toLocaleDateString("en-GB"));
            const carbs = weeklyData.map(item => item.totalCarbs);

            return {
                labels: days,
                datasets: [
                    {
                        label: this.$t('nutrition_charts.carbs'),
