    ...
    methods: {
        async fetchSugarData() {
            ...
            try {
                ...
                console.error('Error fetching weekly sugar data:', error);
            }
        },
        setChartData(weeklyData) {
            const days = weeklyData.map(item => new Date(item.forDay).toLocaleDateString());
            const sugar = weeklyData.map(item => item.totalSugar);

            return {
                labels: days,
                datasets: [
                    {
                        label: this.$t('nutrition_charts.sugar'),
